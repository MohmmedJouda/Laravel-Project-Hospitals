    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title mt-2">standard Table</h3>

                <a href="{{ route('standards.create') }}" class="btn btn-success float-right">Add standard</a>
            ...
            <div class="card-body">
                ...
                <div class="card-body table-responsive p-0">
                    <table class="table table-hover text-nowrap table-bordered">
                        ...
                        <tbody>
                            @foreach ($item as $key => $standard)
                                <tr>
                                    <td>{{ ++$key }}</td>
                                    <td>{{ $standard->chose_standard }}</td>
                                    <td>{{ $standard->title }}</td>
                                    <td>{{ $standard->details }}</td>
                                    <td>{{ $standard->cover }}</td>
                                    <td><img src="{{ Storage::url('standards/' . $standard->cover) }}" alt="Not Found"
                                            width="60" height="60"></td>
                                    <td>{{ $standard->created_at }}</td>
                                    <td>{{ $standard->updated_at }}</td>
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{ route('standards.edit', $standard->id) }}" class="btn btn-info">
                                            ...
                                            <form method="POST" action="{{ route('standards.destroy', $standard->id) }}">
                                                @csrf
                                                @method('DELETE')
